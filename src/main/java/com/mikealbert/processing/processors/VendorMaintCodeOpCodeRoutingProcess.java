...
public class VendorMaintCodeOpCodeRoutingProcess {
	
	private static final String VENDOR_ADD = "direct:vendorMaintCodeAdd";
	private static final String VENDOR_UPDATE = "direct:vendorMaintCodeUpdate";
	private static final String VENDOR_DELETE = "direct:vendorMaintCodeDelete";
	...
	public String determineRoute(@Body Object body, @Properties Map<String, Object> properties){
		...
		if(getInvokedMarker(properties)){
			return null;
		}else if(body instanceof VendorMaintCodeVO && ((VendorMaintCodeVO) body).getOperationCode().equalsIgnoreCase("A")){ 
			setInvokedMarker(properties,true);
			return VENDOR_ADD;
		}else if(body instanceof VendorMaintCodeVO && ((VendorMaintCodeVO) body).getOperationCode().equalsIgnoreCase("M")){ 
			setInvokedMarker(properties,true);
			return VENDOR_UPDATE;
		}else if(body instanceof VendorMaintCodeVO && ((VendorMaintCodeVO) body).getOperationCode().equalsIgnoreCase("D")){ 
			setInvokedMarker(properties,true);
